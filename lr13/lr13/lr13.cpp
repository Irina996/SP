// lr13.cpp : Defines the entry point for the application.
//
...
#define MAINLISTBOX 1
#define SECONDLISTBOX 2

// Global Variables:
HINSTANCE hInst;                                // current instance
WCHAR szTitle[MAX_LOADSTRING];                  // The title bar text
WCHAR szWindowClass[MAX_LOADSTRING];            // the main window class name
HWND mainHWnd, mainListBox, secondListBox;
	...
	_In_ int       nCmdShow)
{
	...
	LoadStringW(hInstance, IDC_LR13, szWindowClass, MAX_LOADSTRING);
	...
	HACCEL hAccelTable = LoadAccelerators(hInstance, MAKEINTRESOURCE(IDC_LR13));
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
	...
	wcex.hIcon = LoadIcon(hInstance, MAKEINTRESOURCE(IDI_LR13));
	wcex.hCursor = LoadCursor(nullptr, IDC_ARROW);
	wcex.hbrBackground = (HBRUSH)(COLOR_WINDOW + 1);
	wcex.lpszMenuName = MAKEINTRESOURCEW(IDC_LR13);
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
	...
	mainListBox = CreateWindow(L"LISTBOX", L"Main", WS_CHILD | WS_VISIBLE | WS_BORDER | WS_VSCROLL | LBS_NOTIFY,
		10, 10, 500, 650, hWnd, (HMENU)MAINLISTBOX, hInstance, NULL);
	secondListBox = CreateWindow(L"LISTBOX", L"Second", WS_CHILD | WS_VISIBLE | WS_BORDER | WS_VSCROLL,
		600, 10, 400, 650, hWnd, (HMENU)SECONDLISTBOX, hInstance, NULL);
...
void UpdateProcesses() {
	...
	for (unsigned int i = 0; i < numberOfProcessesFound; i++) {
		if (allProcesses[i] != 0) {
			...
			if (hProcess != NULL) {
				...
				swprintf_s(finalName, L"%s (PID: %u)", nameOfProcess, allProcesses[i]);
	...
	if (EnumProcessModules(hProcess, hMods, sizeof(hMods), &cbNeeded))
	{
		for (unsigned int i = 0; i < (cbNeeded / sizeof(HMODULE)); i++)
		{
				...
				sizeof(szModName) / sizeof(TCHAR)))
			{
				...
				swprintf_s(finalName, L"\t%s (0x%p)\n", szModName, hMods[i]);
				SendMessage(secondListBox, LB_ADDSTRING, 0, (LPARAM)finalName);
			}
		}
	}
